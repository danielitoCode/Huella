...
function mapDoc(doc: Record<string, unknown>): Solicitud {
  return {
    ...
    motivoCierre: (doc.motivoCierre as string) || null,
    kycResultado: (doc.kycResultado as string) || null,
    diditSessionId: (doc.diditSessionId as string) || null,
    verificationUrl: (doc.verificationUrl as string) || null,
    creadoPorIp: (doc.creadoPorIp as string) || null,
...
export async function listSolicitudes(opts: {
  ...
  const { db, id } = col();
  const databases = getDatabases();
  const queries = [Query.limit(opts.limit ?? 50), Query.offset(opts.offset ?? 0), Query.orderDesc('$createdAt')];
